...
class Mixture:
    ...
    def Plotter(self, BestParams, Model, Fit, ModelInstance, Actual, c, T):
        ...
        (TangentComps, ExitMode, msg) = PhaseStability.CalcPhaseStability(ModelInstance, T, c, self.M)
        ...
        ax.text(0,0, 'Model: %s, Params: %s, Abslote Error(sum): %4.4e, Termination: %s'%(Model, str(around(BestParams, 4)),AbsError, msg), fontsize=12, transform=ax.transAxes)
        ax.set_axis_off()
        #show()
